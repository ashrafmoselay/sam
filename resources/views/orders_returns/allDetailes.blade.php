...
<div class="container">
    <div class="row">
        ...
        <div class="col-md-12">
			...
			<div id="list">
				{!! View::make('returns._listdetails',compact('list'))!!}
			 </div>
		</div>
	</div>
</div>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$(document).on("change",".fromdate,.todate,.supplier_id",function(e){
			...
			var url_ = "{{url('returns./search')}}";
			$.ajax({
				url:url_,
				type:'GET',
				data:{from:from,to:to,supplier_id:supplier_id,view:2},
		...
		$(document).on("change",".page_size",function(e){
			...
			$.ajax({
				url:url_,
				type:'GET',
				data:{page_size:page_size,from:from,to:to,supplier_id:supplier_id,view:2},
				success:function(result){
					 appendParm();
					$("#list").html(result);
	...
	function appendParm(){
		var from = $(".fromdate").val();
		var to = $(".todate").val();
		var newhref = "{{url('printPdf','Orders')}}"+"?supplier_id_id="+$(".supplier_id").val()+"&from="+from+"&to="+to;
